...
export default class Roaming {
	 ...
	 * @param {Object} options.billboard  - 标注  Cesium.billboardGraphics.ConstructorOptions
	 * @param {Object} options.billboard.show  - billboard弹框可见
	...
	modelRoaming(options) {
		...
		this.showBillboard = options.showBillboard
		...
		this.modelData.billboard = Object.assign(billboard, options.billboard);
		...
		this.modelData.cylinder.length = new Cesium.CallbackProperty(
			function () {},
			false
		);
		var property = this._ComputeRoamingLineProperty(this.Lines, this.time);
	...
	_modelInitRoaming(position, start, stop) {
		...
		this.entity = this.viewer.entities.add({
			...
			billboard: this.modelData.billboard,
		});
			...
			if (_this.entity) {
				if (_this.viewer.clock.shouldAnimate) {
					...
					if (_this.modelData.polyline.show) {
						...
						_this.modelData.polyline.positions._callback =
							function () {
								return positions;
							};
					}
					...
					if (_this.ifTileset) {
						_this.entity.position.addSample(
							_this.viewer.clock.currentTime,
							_this.viewer.scene.clampToHeight(center, [
								_this.entity,
							])
						);
					}
					if (_this.modelData.cylinder.show) {
						_this.modelData.cylinder.length._callback =
							function () {
								let length =
									_this.viewer.scene.globe.ellipsoid.cartesianToCartographic(
										center
			...
			this.viewer.zoomTo(
				...
				new Cesium.HeadingPitchRange(
					Cesium.Math.toRadians(
						!options.heading ? 0 : options.heading
					),
	...
	_realTimeData(center) {
		...
		// });
		_this.data.progress =
			((_this.data.roamingLength / _this.data.totalLength) * 100).toFixed(
				0
			) + "%"; //进度
	}
	/**
	...
	_formateSeconds(endTime) {
		...
		secondTime =
			secondTime.toString() == 0 ? "" : secondTime.toString() + "秒";
		result = `${h + min + secondTime}`;
		return result == "" ? "0秒" : result;
	}
	/**
	 * 获取模型在当前轨迹位置上的偏航角度（弧度/度）
	 * @param {Boolean} toDegrees - 是否转换为角度值（默认false返回弧度）
	 * @returns {Number|null} 偏航角度值，如果无法计算则返回null
	 */
	getCurrentYaw(toDegrees = false) {
		if (!this.entity || !this.entity.position) {
			console.warn("没有活动的漫游实体");
			return null;
		}
		...
		try {
			...
			const currentPosition = this.entity.position.getValue(currentTime);
			...
			const nextPosition = this.entity.position.getValue(nextTime);
			...
			// 如果方向向量接近零，则返回null
			if (Cesium.Cartesian3.magnitude(direction) < 1e-10) {
				return null;
			}

			// 计算偏航角（东方向为0，北方向为90度）
			const yawRadians = Math.atan2(direction.y, direction.x);

			// 将结果转换为Cesium的标准（北为0，东为90度）
			let adjustedYaw =
				Cesium.Math.TWO_PI - yawRadians + Cesium.Math.PI_OVER_TWO;

			// 归一化到0-2π范围
			adjustedYaw = Cesium.Math.zeroToTwoPi(adjustedYaw);

			return toDegrees ? Cesium.Math.toDegrees(adjustedYaw) : adjustedYaw;
		} catch (error) {
			console.error("计算偏航角度时出错:", error);
			return null;
		}
	}

	/**
	 * 获取整个轨迹上所有点的偏航角度（批量计算）
	 * @param {Boolean} toDegrees - 是否转换为角度值（默认false返回弧度）
	 * @param {Number} interval - 采样间隔秒数（默认1秒）
	 * @returns {Array} 包含时间戳和对应偏航角度的数组
	 */
	getTrajectoryYawAngles(toDegrees = false, interval = 1) {
		...
		while (Cesium.JulianDate.lessThanOrEquals(currentTime, stopTime)) {
			try {
				...
				const currentPosition =
					this.entity.position.getValue(currentTime);

				// 获取下一时刻位置（使用小的delta时间）
				const nextTime = Cesium.JulianDate.addSeconds(
					currentTime,
					0.1,
					new Cesium.JulianDate()
				...
				if (currentPosition && nextPosition) {
					...
					if (Cesium.Cartesian3.magnitude(direction) > 1e-10) {
						...
						results.push({
							...
							timeJulian: Cesium.JulianDate.clone(currentTime),
							yawRadians: adjustedYaw,
							yawDegrees: toDegrees
								? Cesium.Math.toDegrees(adjustedYaw)
								: adjustedYaw,
				...
				console.warn("计算轨迹偏航角度时出错:", error);
				break;
			}
		}
	...
	/**
	 * 获取指定时间点的偏航角度
	 * @param {Cesium.JulianDate|Date} time - 时间点
	 * @param {Boolean} toDegrees - 是否转换为角度值（默认false返回弧度）
	 * @returns {Number|null} 偏航角度值
	 */
	getYawAtTime(time, toDegrees = false) {
		if (!this.entity || !this.entity.position) {
			return null;
		}
		...
		if (time instanceof Date) {
			julianTime = Cesium.JulianDate.fromDate(time);
		} else if (time instanceof Cesium.JulianDate) {
			julianTime = time;
		} else {
			console.error("时间参数类型不正确");
			return null;
		}

		try {
			// 获取指定时间位置
			const position = this.entity.position.getValue(julianTime);
			...
			// 计算方向向量
			const direction = new Cesium.Cartesian3();
			Cesium.Cartesian3.subtract(nextPosition, position, direction);
			...
			console.error("计算指定时间偏航角度时出错:", error);
			return null;
		}
	}
	...
	/**
	 * 实时监听偏航角度变化
	 * @param {Function} callback - 回调函数，接收偏航角度参数
	 * @param {Boolean} toDegrees - 是否转换为角度值（默认false返回弧度）
	 * @returns {Function} 取消监听函数
	 */
	onYawChange(callback, toDegrees = false) {
		...
		const listener = function () {
			if (_this.entity && _this.viewer.clock.shouldAnimate) {
				const currentYaw = _this.getCurrentYaw(toDegrees);

				// 只有当偏航角度发生变化且有效时才回调
				if (currentYaw !== null && currentYaw !== lastYaw) {
					lastYaw = currentYaw;
					callback(currentYaw);
				}
			}
		...
		// 返回取消监听函数
		return function () {
			_this.viewer.scene.preUpdate.removeEventListener(listener);
		};
	}
}
